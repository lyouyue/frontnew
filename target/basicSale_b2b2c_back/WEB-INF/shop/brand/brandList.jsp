  ...
  <head>
    ...
    $(function(){
		   $("#tt").datagrid({//数据表格
				...
				columns:[[//未冻结字段
					{field:"brandImageUrl",title:"首页推荐品牌图片",width:80,
				   ...
				   {field:"brandName",title:"品牌名称",width:80, formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
		                return "<a style='display:block;' id='"+rowData.brandId+"' onclick='showDetailInfo(this.id);'>"+rowData.brandName+"</a>";  
		         	  }  
					}, 
					{field:"firstWord",title:"品牌归类",width:50},
					...
					{field:"isRecommend",title:"品牌频道推荐",width:50,
						formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
	                        if(value=="0"){ return "<font class='color_002'>否</font>";} 
	                        if(value=="1"){ return "<font class='color_001'>是</font>";} 
                         }
					},
					{field:"isHomePage",title:"店铺首页品牌推荐",width:50,
						formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
	                        if(value=="0"){ return "<font class='color_002'>否</font>";} 
	                        if(value=="1"){ return "<font class='color_001'>是</font>";} 
                         }
				...
				%>	
					...
					iconCls:"icon-add",
					handler:function(){
						createWindow(930,440,"&nbsp;&nbsp;添加商品品牌","icon-add",false,"addOrEditWin");
						$("#brandId").val("");
						$("#brandName").val("");
						$("#title").val("");
						$("#synopsis").val("");
				...
				%>
				{
					...
					handler:function(){
						...
						if(rows.length<=0){
							...
							$("#fileId1").val("");
							$("#fileId2").val("");
							createWindow(930,440,"&nbsp;&nbsp;修改商品品牌","icon-edit",false,"addOrEditWin");
	    			$.ajax({
	    ...
	    function showDetailInfo(id){
			createWindow(800,315,"&nbsp;&nbsp;商品品牌详情","icon-tip",false,"detailWin");
			$.ajax({
				   type: "POST",
				   url: "${pageContext.request.contextPath}/back/brand/getBrandInfo.action",
				   data: {brandId:id},
