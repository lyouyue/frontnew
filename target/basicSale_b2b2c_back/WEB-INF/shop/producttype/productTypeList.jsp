...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function addProductType(id){
			//准备初始数据 ,父亲分类 
			createWindow(900,'auto',"&nbsp;&nbsp;添加商品分类","icon-add",false,"win");
		...
		function editProductType(id){
			$.ajax({
				...
				success: function(data){
					//显示 
					createWindow(900,480,"&nbsp;&nbsp;修改商品分类","icon-edit",false,"win",10);
					$("#fileId1").val("");
					$("#fileId2").val("");
					$("#fileId3").val("");
					$("#addOrEditWin").css("display","");
					...
					$.ajax({
						...
						success: function(data){
							if(data.isSuccess=="true"){
								...
								msgShow("系统提示", "<p class='tipInfo'>此分类下存在子分类或关联商品，无法删除！</p>", "warning");  
							}
						}
					});
				}
			...
			<div class="treeCommonBox">
				<ul id="corp-tree" class="simpleTree">
					<li class="root" id='1' style="margin: 0 0 0 -13px;">
						<ul>
							<li id='2'>
								<span class='folder'> &nbsp;&nbsp;<a style="cursor: pointer;" onclick="listProductTypes(1)">商品分类</a>
								<!-- <a style="margin-left: 10px;cursor:hand;" href="javascript:;"  onclick="initProdutTypeInfo()"><font color="red">同步商品分类&nbsp;》</font> </a> -->
								</span>
								<ul class="ajax">
									<li id='3'>
