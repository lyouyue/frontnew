  ...
  <head>
	...
	$("#tt").datagrid({//数据表格
		...
		toolbar:[
			...
			{//工具条
				...
				iconCls:"icon-add",
				handler:function(){
					createWindow(900,430,"&nbsp;&nbsp;添加商品分类","icon-add",false,"addOrEditWin"); 
				...
				handler:function(){
					...
					if(rows.length==0){//没有选择修改项
						...
						$.ajax({
							...
							success: function(data){
								createWindow(900,430,"&nbsp;&nbsp;修改商品分类","icon-edit",false,"addOrEditWin",10);
		...
		if("init".equals((String)functionsMap.get(purviewId+"_init"))){
			%>
			{
				...
				iconCls:"icon-reload",
				handler:function(){
					$.messager.confirm("系统提示", "<p class='tipInfo'>确定同步商品分类到系统内存中吗?</p>",function(data){
						if(data){//判断是否删除
	...
	function showDetailInfo(id){
		$.ajax({
			...
			dataType: "JSON",
			success: function(data){
				createWindow(800,420,"&nbsp;&nbsp;商品分类详情","icon-add",false,"detailWin",10);
