...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    $(function(){
		   $("#tt").datagrid({//数据表格
				...
				url:"${pageContext.request.contextPath}/back/point/listPoint.action",
				queryParams:{pageSize:pageSize},
				idField:"pointId",//唯一性标示字段
				frozenColumns:[[//冻结字段
				    {field:"ck",checkbox:true}
				]],
				columns:[[//未冻结字段
		            {field:"pointId",title:"金币编号",width:120, formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
		                return "<a style='display:block;' id='"+rowData.pointId+"' onclick='showDetailInfo(this.id);'>"+rowData.pointId+"</a>";
		         	  }
					},
					{field:"minNumber",title:"最小订单金额",width:120},
					{field:"maxNumber",title:"最大订单金额",width:120},
					{field:"returnPoint",title:"给予金币值",width:120},
					{field:"endTimestamp",title:"使用结束时间",width:120},
					{field:"updName,delName",title:"操作",width:120,
						formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
		               		return "<a id='"+rowData.pointId+"' onclick='updOrDelSA(this.id,"+rowData.pointId+",1);'>"+"修改"+"</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a id='"+rowData.pointId+"' onclick='updOrDelSA(this.id,"+rowData.pointId+",2);'>"+"删除"+"</a>";
		         	  	}
					}
				]],
								...
								if(data){//判断是否删除
									if (rows){//判断是否找到当前行，找到后异步形式从数据库中删除当前记录
										...
										for(i=0;i<rows.length;i++){
											if(i==rows.length-1)ids+=rows[i].pointId;
											else ids+=rows[i].pointId+",";
										}
										$.ajax({
										   type: "POST",dataType: "JSON",
										   url: "${pageContext.request.contextPath}/back/point/deletePoint.action",
										   data: {ids:ids},
		...
		function query(){
			  queryParams={selectFlag:true,pageSize:pageSize,currentPage:currentPage,promotionId:$("#paramePromotionId").val()};
    		...
    		if(flag == 1){
    			...
    			$.ajax({
					   ...
					   success: function(data){
						   ...
						   $("#minNumber").val(data.minNumber);
						   $("#maxNumber").val(data.maxNumber);
						   $("#returnPoint").val(data.returnPoint);
						   $("#endTimestamp").val(data.endTimestamp);
					   }
					});
    		}else{
  ...
  <body>
  		<span style="font-size: 15px;">促销活动：
  			<select id="paramePromotionId">
		...
		<span ><a href="javascript:query();" id="btn1" iconCls="icon-search" >查询</a></span>
  		...
  		<div id="win" style="display:none;overflow-y:auto;scrollbar-face-color:#C4E1FF;">
		   <!-- 添加或者修改 -->
		  <jsp:include page="addOrEditPoint.jsp"/>
		  <!-- 详情页 -->
		  <jsp:include page="detailPoint.jsp"/>
		</div>
  </body>
</html>
