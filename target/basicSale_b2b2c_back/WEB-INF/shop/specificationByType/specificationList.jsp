...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
	...
	$(function(){
			...
			var defHtml="";
			$("#tt").datagrid({//数据表格
				/* title:"商品规格列表信息:<font color='blue'>${name}</font>",
				iconCls:"icon-save",  */
				width:"auto",
				height:"auto",
				...
				url:"${pageContext.request.contextPath}/back/specificationTree/listSpecification.action?productTypeId="+productTypeId,
				...
				columns:[[//未冻结字段
			         {field:"name",title:"规格名称",width:100, formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
					...
					/* {field:'type',title:'规格类型',width:100, 
                 	...
                 	{field:"productTypeName",title:"分类名称",width:120},
				...
				%>         
			      {//工具条
					...
					iconCls:"icon-add",
					handler:function(){
						createWindow(900,450,"&nbsp;&nbsp;添加商品规格","icon-add",false,"addOrEditWin",10);
						$("#addOrEditWin").css("display","");
						$("#detailWin").css("display","none");
						$("#ggz").css("display","");
						$("#specificationId").val(null);
						$("#especificationId").val(null);
						$("#nameValue0").val(null);
						$("#specificationValueId0").val(null);
						$("#sortValue0").val(1);//默认是1
						$(".addSpecValue").remove();
						$("#productTypeId").val(productTypeId);
						$("#sproductTypeId").val(productTypeId);
						$("#sproductTypeId").attr("disabled","disabled");
				...
				%>
				{
					...
					handler:function(){
						...
						if(rows.length<=0){
							...
							createWindow(900,450,"&nbsp;&nbsp;修改商品规格","icon-edit",false,"addOrEditWin",10);
							...
							$.ajax({
								...
								success: function(data){
									...
									$("#sproductTypeId").attr("disabled","disabled");
									$("#firstListHtml").html("");
									$("#especificationId").val(data.specification.specificationId);
									initSpecificationValue(data.specificationValueList);
								}
							});
						}
					}
	...
	function query(){
		 queryParams={selectFlag:true,pageSize:pageSize,currentPage:currentPage,specificationName:$.trim($("#qspecificationName").val())};
