    ...
    submitHandler:function(form){
        $(document).ready(
			...
			var rows = $("#shopInfoList").datagrid("getSelections");//找到所有选中的行
				if(rows.length<=0){
					msgShow("系统提示", "<p class='tipInfo'>请选择一个店铺！</p>", "warning");
				}else if(rows.length>1){
					msgShow("系统提示", "<p class='tipInfo'>只能选择一个店铺</p>", "warning");
				}else{
					if(rows){
						var shopInfoIds="";
