	...
	$(function(){
		$("#tt").datagrid({//数据表格
			...
			columns:[[//未冻结字段
				...
				{field:"name",title:"商品名称",width:220, formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
			...
			{//工具条
				...
				handler:function(){
					...
					/**表单信息重置 end**/
					createWindow(1039,480,"&nbsp;&nbsp;添加自营店铺商品","icon-edit",false,"merchandise_add",10);
			...
			{//工具条
				...
				handler:function(){
					...
					if(rows[0].isPutSale==2){
						...
						createWindow(1039,480,"&nbsp;&nbsp;修改自营店铺商品","icon-edit",false,"editProductWin",10);
						/** 设置easyuiTabs的样式 begin**/
						$(".tabs-wrap").removeAttr("style");
						$(".tabs-wrap").attr("style","margin-left: 0px; margin-right: 0px; left: 0px; width:100%;");
						$(".setWidth").each(function(i) {
			...
			%>
			{
				...
				handler:function(){
					...
					if(rows.length<=0){
						...
						$.messager.confirm("系统提示", "<p class='tipInfo'>商品将被永久删除、无法恢复！</p>",function(data){
							if(data){//判断是否删除
			...
			{//工具条
				...
				handler:function(){
					...
					if(rows.length==0){//没有选择修改项
						...
						createWindow(1000,'auto',"&nbsp;&nbsp;审核自营店铺商品","icon-tip",false,"addOrEditWin");
						$("#addShopInfo").css("display","none");
						$("#merchandise_add").css("display","none");
						$("#detailWin").css("display","none");
						$("#showInListWin").css("display","none");
				...
				{text:"修改返利比例",
					...
					handler:function(){
						...
						if(rows.length==0){//没有选择修改项
							...
							var id = rows[0].productId;
							createWindow(600,'auto',"&nbsp;&nbsp;修改自营店铺商品返利比例","icon-tip",false,"updateRatioWin");
							$("#addShopInfo").css("display","none");
							$("#merchandise_add").css("display","none");
							$("#detailWin").css("display","none");
							$("#showInListWin").css("display","none");
					...
					$.messager.confirm("系统提示", "<p class='tipInfo'>您确定要重新生成二维码吗？</p>",function(data){
						if(data){//判断是否删除
							...
							$.ajax({
								...
								data: {shopInfoProductType:"1"},//1、平台基础商品；2 、平台店铺商品
								success: function(data){
				...
				/* {text:"列表显示",
					...
					handler:function(){
						...
						if(rows.length==0){//没有选择修改项
							...
							msgShow("系统提示", "<p class='tipInfo'>只能够选择一项商品！</p>", "warning");
						}else if(rows.length==1){
							$("#addOrEditWin").css("display","none");
							$("#detailWin").css("display","none");
							$("#addShopInfo").css("display","none");
