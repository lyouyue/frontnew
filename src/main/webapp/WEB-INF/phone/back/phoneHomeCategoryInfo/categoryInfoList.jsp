  ...
  <head>
    ...
    $(function(){
		   $("#tt").datagrid({//数据表格
				...
				url:"${pageContext.request.contextPath}/back/phoneHomeCategoryInfo/listPhoneHomeCategoryInfo.action?categoryId="+'${categoryId}',
			    ...
			    %>     
			    {
					...
					handler:function(){
						...
						var cid='${categoryId}';
						$("#categoryId").val(cid);
						createWindow(800,430,"&nbsp;&nbsp;添加手机首页分类数据","icon-add",false,"addOrEditWin",10);
					}
				},"-",
				<%
				}
					...
					handler:function(){
						...
						if(rows.length==0){
							...
							$.ajax({
								   ...
								   url: "${pageContext.request.contextPath}/back/phoneHomeCategoryInfo/getPhoneHomeCategoryInfoObj.action",
								   data: {id:rows[0].categoryInfoId},
								   success: function(data){
									   $("#fileId1").val("");
									   createWindow(800,450,"&nbsp;&nbsp;修改手机首页分类信息","icon-edit",false,"addOrEditWin");
							...
							$.messager.confirm("系统提示", "<p class='tipInfo'>你确定要删除吗?</p>",function(data){
								if(data){//判断是否删除
									if (rows){//判断是否找到当前行，找到后异步形式从数据库中删除当前记录
										...
										$.ajax({
										   type: "POST",dataType: "JSON",
										   url: "${pageContext.request.contextPath}/back/phoneHomeCategoryInfo/deleteByIds.action",
										   data: {ids:ids},
										   success: function(data){
		...
		function showDetail(id){
			$.ajax({
				   ...
				   data: {id:id},
				   success: function(data){
						createWindow(800,450,"&nbsp;&nbsp;手机首页分类信息详情","icon-search",false,"detailWin");
