  ...
  <head>
    ...
    $(function(){
    	$("#tt").datagrid({//数据表格
			...
			url:"${pageContext.request.contextPath}/back/shopSalesStatistics/listShopSalesStatistics.action", 
			idField:"shopInfoId",//唯一性标示字段
			columns:[[//未冻结字段
	           {field:"logoImg",title:"店铺LOGO",width:50, formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
					return "<img src='<s:property value='%{#application.fileUrlConfig.uploadFileVisitRoot}'/>"+rowData.logoUrl+"' onerror='this.src=\"${fileUrlConfig.sysFileVisitRoot_back}productInfo/images/mrbj.png\"' width='30px' height='30px' />";  
				  }
				},
			...
			%>         
			{
				text:"导出报表",
	 			iconCls:"icon-redo",
	 			handler:function(){
	 				window.location.href="${pageContext.request.contextPath}/back/shopSalesStatistics/exportCountModFuleExcel.action?createTime="+$("#qcreateTime").val()+"&createTime2="+$("#qcreateTime2").val()+"&shopName="+$("#qshopName").val();
	 			}
			},"-",
			<% 
			}
		...
		pageSplit(pageSize);//调用分页函数
 	 	...
 	 	function query(){
 	  	  	queryParams={selectFlag:true,pageSize:pageSize,currentPage:currentPage,shopName:$("#qshopName").val(),createTime:$("#qcreateTime").val(),createTime2:$("#qcreateTime2").val()};
  ...
  <div class="main">
	...
	<input id="ids"  type="hidden" name="ids"/>
		...
		<table   class="searchTab">
			<tr>
				<td class="toright_td" style="width: 85px;">店铺名称：</td>
