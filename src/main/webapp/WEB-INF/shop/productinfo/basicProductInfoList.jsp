  ...
  <head>
	...
	$(function(){
		$("#tt").datagrid({//数据表格
			...
			url:"${pageContext.request.contextPath}/back/productinfo/getBasicProductList.action",
			...
			columns:[[//未冻结字段
				{field:"logoImg",title:"套餐图片",width:80, formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
				...
				{field:"name",title:"套餐名称",width:220, formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
				...
				{field:"productCode",title:"套餐编号",width:100},
			...
			{//工具条
				...
				iconCls:"icon-add",
				handler:function(){
					//window.open("${pageContext.request.contextPath}/back/productinfo/addProductInfo.action","mainFrame","width="+(window.screen.availWidth-10)+",height="+(window.screen.availHeight-30)+ ", top=0,left=0,toolbar=no, menubar=yes, scrollbars=yes,fullscreen=yes, resizable=yes,location=yes, status=no");
					...
					//$(".tabs li:first").attr("class","tabs-selected");
					/**表单信息重置 end**/
					createWindow(1039,480,"&nbsp;&nbsp;添加自营机构套餐","icon-edit",false,"merchandise_add",10);
					$("#addTab").tabs("select","基本信息");//默认选中基本信息
			...
			{//工具条
				...
				handler:function(){
					...
					if(rows[0].isPutSale==2){
						msgShow("系统提示", "<p class='tipInfo'>上架套餐不能修改！</p>", "warning");
					}else if(rows.length==1){
						//window.open("${pageContext.request.contextPath}/back/productinfo/getProductInfoByProductId.action?productId="+rows[0].productId+"&currentPage="+currentPage,"mainFrame","width="+(window.screen.availWidth-10)+",height="+(window.screen.availHeight-30)+ ", top=0,left=0,toolbar=no, menubar=yes, scrollbars=yes,fullscreen=yes, resizable=yes,location=yes, status=no");
						$("#addOrEditWin").css("display","none");
						$("#detailWin").css("display","none");
						$("#addShopInfo").css("display","none");
						$("#showInListWin").css("display","none");
						...
						createWindow(1039,480,"&nbsp;&nbsp;修改自营机构套餐","icon-edit",false,"editProductWin",10);
						/** 设置easyuiTabs的样式 begin**/
						$(".tabs-wrap").removeAttr("style");
						$(".tabs-wrap").attr("style","margin-left: 0px; margin-right: 0px; left: 0px; width:100%;");
						$(".setWidth").each(function(i) {
						...
						/**设置easyuiTabs的样式 end**/
						$("#editTab").tabs("select","基本信息");//默认选中基本信息
			...
			%>
			{
				...
				handler:function(){
					var rows = $("#tt").datagrid("getSelections");//找到所有选中的行
					if(rows.length<=0){
						msgShow("系统提示", "<p class='tipInfo'>请选择套餐！</p>", "warning");
					}else{
						$.messager.confirm("系统提示", "<p class='tipInfo'>套餐将被永久删除、无法恢复！</p>",function(data){
							if(data){//判断是否删除
			...
			{//工具条
				...
				handler:function(){
					...
					if(rows.length==0){//没有选择修改项
						...
						if(isPass==2){//待申请
							msgShow("系统提示", "<p class='tipInfo'>此套餐处于待申请状态，不能审核！</p>", "warning");
							return false;
						}
						var id = rows[0].productId;
						createWindow(1000,'auto',"&nbsp;&nbsp;审核自营机构套餐","icon-tip",false,"addOrEditWin");
						$("#addShopInfo").css("display","none");
						$("#merchandise_add").css("display","none");
						$("#detailWin").css("display","none");
						$("#showInListWin").css("display","none");
				...
				{text:"修改返利比例",
					...
					handler:function(){
						...
						if(rows.length==0){//没有选择修改项
							...
							var isPass = rows[0].isPass;
							if(isPass==2){//待申请
								msgShow("系统提示", "<p class='tipInfo'>此套餐处于待申请状态，不能修改返利比例！</p>", "warning");
								return false;
							}
							var id = rows[0].productId;
							createWindow(600,'auto',"&nbsp;&nbsp;修改自营机构套餐返利比例","icon-tip",false,"updateRatioWin");
							$("#addShopInfo").css("display","none");
							$("#merchandise_add").css("display","none");
							$("#detailWin").css("display","none");
							$("#showInListWin").css("display","none");
					...
					$.messager.confirm("系统提示", "<p class='tipInfo'>您确定要重新生成二维码吗？</p>",function(data){
						if(data){//判断是否删除
							...
							$.ajax({
								...
								data: {shopInfoProductType:"1"},//1、平台基础套餐；2 、平台机构套餐
								success: function(data){
				...
				/* {text:"列表显示",
					...
					handler:function(){
						...
						if(rows.length==0){//没有选择修改项
							...
							msgShow("系统提示", "<p class='tipInfo'>只能够选择一项套餐！</p>", "warning");
						}else if(rows.length==1){
							$("#addOrEditWin").css("display","none");
							$("#detailWin").css("display","none");
							$("#addShopInfo").css("display","none");
							$("#merchandise_add").css("display","none");
							$("#editProductWin").css("display","none");
							$("#showInListWin").css("display","");
							createWindow(500,'auto',"&nbsp;&nbsp;列表显示","icon-edit",false,"showInListWin");
						}
					}
				},"-", */
			{text:"刷新",
...
<body>
  ...
  <div class="main">
	   <table class="searchTab">
		<tr>
			<%-- <td class="search_toright_td" style="width: 65px;">套餐分类：</td>
			<td class="search_toleft_td" style="width:115px;">
				<select id="qproductTypeId" style="width:105px;" class="querySelect">
				...
				<td class="search_toright_td" style="width:75px;">套餐名称：</td>
				<td class="search_toleft_td" style="width:120px;"><input type="text" style="width:105px;" id="qprodName" name="qprodName"  class="searchTabinput"/>&nbsp;&nbsp;</td>
				<td class="search_toright_td"  style="width:65px;">SKU号：</td>
				<td class="search_toleft_td" style="width:120px;"><input type="text" style="width:105px;" id="qsku" name="sku" class="searchTabinput"/>&nbsp;&nbsp;</td>
				
				<td class="search_toright_td" style="width:65px;">审核状态：</td>
				<td class="search_toleft_td" style="width:115px;">
					<select id="qisPass" style="width:105px;" class="querySelect">
				...
				<td class="search_toright_td" style="width: 65px;">是否上架：</td>
				<td class="search_toleft_td" style="width:125px;">
					<select id="qisPutSale" style="width:110px;">
						<option value="-1">--请选择--</option>
		...
		<div id="win" style="display:none;scrollbar-face-color:#C4E1FF;">
			...
			<!-- 审核-->
			<jsp:include page="auditing.jsp"/>
			<!-- 详情页 -->
			<jsp:include page="detail.jsp"/>
			<!-- 快速的分类到机构将套餐 -->
			<jsp:include page="addShopInfo.jsp"/>
			<!-- 添加套餐 -->
			<jsp:include page="merchandise_add_new.jsp"/>
			<!-- 修改套餐 -->
