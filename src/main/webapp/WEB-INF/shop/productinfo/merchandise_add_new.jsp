			...
			submitHandler: function(form) {
				if($("#productType-span-3").find("option:selected").length>0){
					if($("#productType-span-3").find("option:selected").attr("loadType")=="1"){
						...
						msgShow("系统提示", "<p class='tipInfo'>请选择套餐分类！</p>", "warning");  
					}
				}else{
					msgShow("系统提示", "<p class='tipInfo'>请选择套餐分类！</p>", "warning");  
				}
	...
	function validateSubmit(){
		...
		if(isRepeats){
			...
			if(address==""){
				...
				msgShow("系统提示", "<p class='tipInfo'>基本信息，套餐名称必填！</p>", "warning");  
				isRepeats = false;
				return isRepeats;
			}else if($("#brandId").val()==""){
				...
				msgShow("系统提示", "<p class='tipInfo'>基本信息，套餐Logo必填！</p>", "warning");  
				isRepeats = false;
				return isRepeats;
	...
	function changePrice(isRepeats){
		...
		if(isRepeats){
			...
			$("input[name='marketPrice']").each(function(i) {
				if(!($(this).val()!=null&&$(this).val()!='')){
					msgShow("系统提示", "<p class='tipInfo'>套餐规格，市场价格必填！</p>", "warning");  
					isRepeats = false;
					return false;
				}else{
		...
		if(isRepeats){
			...
			$("input[name='salesPrice']").each(function(i) {
				if(!($(this).val()!=null&&$(this).val()!='')){
					msgShow("系统提示", "<p class='tipInfo'>套餐规格，销售价格必填！</p>", "warning");  
					isRepeats = false;
					return false;
				}else{
		...
		if(isRepeats){
			...
			$("input[name='storeNumber']").each(function(i) {
				if(!($(this).val()!=null&&$(this).val()!=''&&$(this).val()>0)){
					msgShow("系统提示", "<p class='tipInfo'>套餐规格，库存必填！</p>", "warning");  
					isRepeats = false;
					return false;
				}
			});
			...
			if(salesPriceArray.length==marketPriceArray.length){
				for(var j=0;j<salesPriceArray.length;j++){
					if(Number(salesPriceArray[j])>Number(marketPriceArray[j])){//销售价大于市场价
						msgShow("系统提示", "<p class='tipInfo'>套餐规格，规格相同的销售价格不能大于其市场价格！</p>", "warning");  
						isRepeats = false;
						break;
					}
				}
					...
					if(data!=null){
						...
						if (jaListProductAttr !=""&&jaListProductAttr!=null) {
							...
							if (listProductAttr.length > 0) {
								var htmlAttrStr = "<tr><strong>套餐属性</strong></tr>";
			...
			<div id="product_rightBox" >
				<div class="easyui-tabs" id="addTab">
					<div title="基本信息"  class="addProductByPT setWidth">
						<table style="margin:11px 11px 11px 11px;">
							<tr>
							...
							<tr>
								<th class="toright_td"><i class="ColorRed">*</i>&nbsp;套餐分类：</th>
								<td colspan="3">
							...
							<tr>
								...
								<th class="toright_td"><i class="ColorRed">*</i>&nbsp;套餐品牌：</th><td id="brandTd"></td>
								<th class="toright_td"><i class="ColorRed">*</i>&nbsp;计量单位：</th><td id="measuringUnit"></td>
							</tr>
							<tr>
								<th class="toright_td"><i class="ColorRed">*</i>&nbsp;套餐编号：</th><td><input   type="text" id="productCode" name="productInfo.productCode" class="{validate:{required:true,maxlength:[30]}} us_input"  /></td>
							...
							<tr>
								...
								<th class="toright_td"><i class="ColorRed">*</i>&nbsp;套餐库存：</th><td><input   type="text" id="storeNumber" name="productInfo.storeNumber" class="{validate:{required:true,number:true,maxlength:[8]}} us_input"/></td>
							</tr>
							<tr>
								...
								<th class="toright_td">&nbsp;套餐重量：</th><td><input id="weight" type="text" name="productInfo.weight" />&nbsp;&nbsp;&nbsp;kg<span id="measuringUnitName1"></span></td>
								<th class="toright_td">&nbsp;包装规格：</th><td><input type="text" id="packingSpecifications" name="productInfo.packingSpecifications" /></td>
							</tr>
							<tr>
								<th class="toright_td"><i class="ColorRed">*</i>&nbsp;上一级返利：</th><td><input type="text" id="a_upRatio" name="productInfo.upRatio" class="{validate:{required:true,number:true,max:99,min:1}} us_input" value="<s:property value="productInfo.upRatio" />"/>&nbsp;&nbsp;&nbsp;%</td>
								<th class="toright_td"><i class="ColorRed">*</i>&nbsp;上二级返利：</th><td><input type="text" id="a_secRatio" name="productInfo.secRatio" class="{validate:{required:true,number:true,max:99,min:1}} us_input" value="<s:property value="productInfo.secRatio" />"/>&nbsp;&nbsp;&nbsp;%</td>
								<th class="toright_td"><i class="ColorRed">*</i>&nbsp;上三级返利：</th><td><input type="text" id="a_thiRatio" name="productInfo.thiRatio" class="{validate:{required:true,number:true,max:99,min:1}} us_input" value="<s:property value="productInfo.thiRatio" />"/>&nbsp;&nbsp;&nbsp;%</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<th class="toright_td">&nbsp;制造商型号：</th><td><input   type="text" id="manufacturerModel" name="productInfo.manufacturerModel" />
									<input type="hidden" id="isChargeFreight" name="productInfo.isChargeFreight" value="1" />
								</td>
								<th class="toright_td">套餐备提示：</th><td><input   type="text" name="productInfo.note" class="{validate:{maxlength:[1000]}} us_input"/></td>
							</tr>
							<tr>
					...
					<!-- 套餐规格 begin-->
					<div title="套餐规格" class="addProductByPT">
						<!-- 添加规格-->
				...
				<div style="padding:0px;">
					<div class="editButton"  data-options="region:'south',border:false" >
						<a id="add_btnSave" class="easyui-linkbutton" icon="icon-save" onclick="javascript:saveProduct();" href="javascript:;">保存</a>
