  ...
  <head>
	...
	$(function(){
		$("#tt").datagrid({//数据表格
				...
				{//工具条
					...
					handler:function(){
						...
						createWindow(1039,480,"&nbsp;&nbsp;添加入驻机构套餐","icon-edit",false,"merchandise_add",10);
						$("#addProduct").css("display","none");
						$("#selectShop").css("display","");
						//控制保存按钮可用
						$("#add_btnSave").attr("onclick","javascript:saveProduct();");
						resetShopInfo();//重置机构查询参数
						$("#aShopInfoId").val("");//去掉机构Id
						$("#aShopInfoName").html("");//去掉机构名称
						$("#shopInfott").datagrid({//数据表格
							...
							columns:[[//未冻结字段
								{field:"logoUrl",title:"机构Logo",width:70, formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
								...
								{field:"shopCategoryName",title:"机构分类",width:70},
				...
				%>
				{
					...
					handler:function(){
						var rows = $("#tt").datagrid("getSelections");//找到所有选中的行
						if(rows.length<=0){
							msgShow("系统提示", "<p class='tipInfo'>请选择已下架套餐！</p>", "warning");
						}else{
									...
									if (rows){//判断是否找到当前行，找到后异步形式从数据库中删除当前记录
										...
										$.ajax({
										   ...
										   success: function(data){
											   if(data){
												   msgShow("系统提示", "<p class='tipInfo'>删除套餐成功！</p>", "warning");
												   $("#tt").datagrid("clearSelections");//删除后取消所有选项
												   $("#tt").datagrid("reload");
											   }else{
												   msgShow("系统提示", "<p class='tipInfo'>删除套餐失败！</p>", "warning");
											   }
										   }
										});
									}	
				...
				{text:"修改返利比例",
					...
					handler:function(){
						...
						if(rows.length==0){//没有选择修改项
							...
							var id = rows[0].productId;
							createWindow(600,'auto',"&nbsp;&nbsp;修改入驻机构套餐返利比例","icon-tip",false,"updateRatioWin");
						...
						$.messager.confirm("系统提示", "<p class='tipInfo'>您确定要重新生成二维码吗？</p>",function(data){
							if(data){//判断是否删除
								...
								$.ajax({
									...
									data: {shopInfoProductType:"2"},//1、平台基础套餐；2 、平台机构套餐
									success: function(data){
		...
		function showEditWin(rows){
			...
			createWindow(1039,480,"&nbsp;&nbsp;修改入驻机构套餐","icon-edit",false,"editProductWin",10);
