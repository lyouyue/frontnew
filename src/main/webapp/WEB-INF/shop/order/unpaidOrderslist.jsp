  ...
  <head>
	...
	$(function(){
			$("#tt").datagrid({//数据表格
				...
				queryParams:{pageSize:pageSize, settlementStatus:1, orderState: 0},
				idField:"ordersId",//唯一性标示字段
				frozenColumns:[[//冻结字段
					{field:"ck",checkbox:true}
				]],
				columns:[[//未冻结字段
					{field:"title",title:"订单号",width:180, formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
					...
					{field:"shopInfoType",title:"机构类型",width:63,formatter:function(value,rowData,rowIndex){
						   if(value==1){
							    return"自营机构";
						   }else if(value==2){
							    return"入驻机构";
					...
					{field:"consignee",title:"体检人名称",width:80},
...
<%--					{field:"address",title:"收货地址",width:200},--%>
					...
					{field:"updateTime",title:"订单付款时间",width:140, formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
						return toJsonDate(value,"yyyy-MM-dd hh:mm:ss");
					}},
					{field:"shopInfoType",title:"操作",width:80,
						formatter:function(value,rowData,rowIndex){
							if(value==1){
								return "<a id='"+rowData.ordersId+"' onclick='updateOrdersStuts(this.id);'>配货</a>";
							}else{
								return "--";
							}
						}
						...
						if(rows.length==0){//没有选择修改项
							...
							$.ajax({
								...
								success: function(data){
									...
									html += "<tr><td align='center' width='150px'>图片<\/td><td align='center' width='150px'>套餐名称<\/td><td align='center' width='150px'>销售价<\/td><td align='center' width='150px'>运费<\/td>"+
							...
							createWindow(800,450,"&nbsp;&nbsp;查看订单套餐","icon-search",false,"view_Order_Products");
		...
		//修改订单状态为配货
		function updateOrdersStuts(ordersId){
			$.ajax({
				type: "POST",
				dataType: "JSON",
				url: "${pageContext.request.contextPath}/back/orders/saveOrUpdateOrdersStuts.action",
				data: {ordersId:ordersId},
				success: function(data){
					if(data.isSuccess){
						$("#tt").datagrid("clearSelections");//删除后取消所有选项
						$("#tt").datagrid("reload"); //保存后重新加载列表
						msgShow("系统提示", "<p class='tipInfo'>配货成功！</p>", "warning");
					}else{
						msgShow("系统提示", "<p class='tipInfo'>配货失败，请重试！</p>", "warning");
					}
				}
			});
		}
	...
	<jsp:include page="../../util/item.jsp"/>
		...
		<table class="searchTab">
			<tr>
				...
				<td class="search_toright_td" style="width:70px;;">机构类型：</td>
				<td class="search_toleft_td" style="width:120px;">
					<select id="qshopInfoType" class="querySelect">
						<option value="">--请选择--</option>
						<option value="1">自营机构</option>
						<option value="2">入驻机构</option>
					</select>
				</td>
