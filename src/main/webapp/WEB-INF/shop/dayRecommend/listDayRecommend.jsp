	...
	$(function(){
    	...
    	$("#form2").validate({meta: "validate", 
		   ...
		   $("#tt").datagrid({//数据表格
				...
				columns:[[//未冻结字段
				      {field:"productCode",title:"套餐编号",width:30},
			          {field:"productName",title:"套餐名称",width:150,formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
			                return "<a style='display:block;' id='"+rowData.id+"' onclick='showDetailInfo(this.id);'>"+rowData.productName+"</a>";  
			         	  }
			            },
		            {field:"logoImages",title:"套餐图片",width:80,formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
		                return "<a style='display:block;' id='"+rowData.id+"' onclick='showDetailInfo(this.id);'><img src='"+"<s:property value='%{#application.fileUrlConfig.image_brand}'/>"+value+"' onerror='this.src=\"${fileUrlConfig.sysFileVisitRoot_back}productInfo/images/mrbj.png\"' width='30px' height='25px' /></a>"; 
	            		}
		            }, 
				...
				toolbar:[//工具条
					...
					handler:function(){
						...
						$("#id").val("");
						createWindow(900,"auto","&nbsp;&nbsp;添加推荐套餐","icon-add",false,"addWin");
						$("#typeId").val("16");
						...
						$("#productList").datagrid({//数据表格
							...
							columns:[[//未冻结字段
					            {field:"logoImg",title:"套餐图片",width:30,formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
					               return "<img src='"+"<s:property value='%{#application.fileUrlConfig.image_brand}'/>"+value+"' onerror='this.src=\"${fileUrlConfig.sysFileVisitRoot_back}productInfo/images/mrbj.png\"' width='30px' height='25px' />"; 
					            	}
					            },
					            {field:"productFullName",title:"套餐名称",width:170},
					            {field:"productCode",title:"套餐编号",width:30}, 
					            {field:"marketPrice",title:"原价(元)",width:40}, 
					            {field:"salesPrice",title:"销售价(元)",width:40},
					            {field:"isShow",title:"显示类型",width:55,
					            	formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
				                       return "<font class='color_001'>列表主套餐</font>";
			                         }
					            }
							]],
							pagination:true,//显示分页栏
				...
				%>
				{
					...
					handler:function(){
						...
						if(rows.length<=0){
							...
							$("#detailWin").css("display","none");
							$("input.button_save").removeAttr("disabled");//防止重复提交
							createWindow(700,"auto","&nbsp;&nbsp;修改推荐套餐","icon-edit",false,"editWin");
							$.ajax({
								   ...
								   success: function(data){
									   $("#id").val(data.id);
									   $("#logoImg").html("<img src='"+"http://www.qydjk.org/qypaasImg/"+data.logoImg+"' onerror='this.src=\"${fileUrlConfig.sysFileVisitRoot_back}productInfo/images/mrbj.png\"' width='40px' height='40px' />");
	...
	<jsp:include page="../../util/item.jsp"/>
		...
		<div class="main">
			<table class="searchTab">
				<tr>
					<td class="search_toright_td" style="width:85px;">套餐名称：</td>
					<td class="search_toleft_td" style="width:140px;">
						<input type="text" id="qproductName"  name="s_productName"/>
					</td>
					<td class="search_toright_td" style="width:80px;">套餐编号：</td>
