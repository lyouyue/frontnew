  ...
  <head>
    ...
    $(function(){
		   ...
		   $("#tt").datagrid({//数据表格
				...
				columns:[[//未冻结字段
					 {field:"productPointId",title:"套餐金币编号",width:120, formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
		                return "<a style='display:block;' id='"+rowData.productPointId+"' onclick='showDetailInfo(this.id);'>"+rowData.productPointId+"</a>";
		         	  }
					},
					{field:"proName",title:"套餐名称",width:120},
				...
				toolbar:[{//工具条
					...
					iconCls:"icon-add",
					handler:function(){
						createWindow(800,'auto',"&nbsp;&nbsp;添加套餐","icon-add",false,"addOrEditWin");
						$("#addOrEditWin").css("display","");
						$("#form1").css("display","");
						$("#form2").css("display","none");
						$("#detailWin").css("display","none");
						getProductInfoList();//获取套餐信息
					}
				},"-",{
					text:"删除",
					iconCls:"icon-remove",
    	...
    	function getProductInfoList(){
			 ...
			 $.ajax({//获取MES牌号信息
				   ...
				   dataType: "JSON",
				   success: function(data){
					   var htmlStr="<tr style='background-color: #C4E1FF;font-weight: bolder;'><td width='8px' class='toleft_td'><input type='checkbox' onclick='checkedAll(this)'></td><td width='250px' class='toleft_td'>&nbsp;&nbsp;套餐名称</td><td width='150px' class='toleft_td'>&nbsp;&nbsp;金币值</td></tr>";
