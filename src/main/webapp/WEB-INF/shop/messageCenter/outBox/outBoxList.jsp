...
$(function(){
		...
		submitHandler:function(form){
			$(document).ready(function(){  
	        	...
	        	if($("#toMemberId2").is(":checked")){
			    	var str = trim($("#toMemberId").val());
			    	$("#toMemberId2").val(str); 
			    	if(str == $("#toMemberId")[0].defaultValue){
			    		flag = false;
			    	}
	            }
                var options = {  
	...
	$("#tt").datagrid({//数据表格
		...
		url:"${pageContext.request.contextPath}/back/messageCenter/listSendMessage.action",
		idField:"messageId",//唯一性标示字段
		frozenColumns:[[//冻结字段
			{field:"ck",checkbox:true}
		]],
		columns:[[//未冻结字段
			{field:"toMemberName",title:"收件人",width:50,formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
		    		return rowData.toMemberName;  
		    	}  
			}, 
			...
			{field:'createDate',title:'发送时间 ',width:50,formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
		...
		{//工具条
			...
			iconCls:"icon-add",
			handler:function(){
				createWindow(750,450,"&nbsp;&nbsp;添加","icon-add",false,"addOrEditWin");
						...
						if(data){//判断是否删除
							if (rows){//判断是否找到当前行，找到后异步形式从数据库中删除当前记录
								...
								$.ajax({
									type: "POST",
									dataType: "JSON",
									url: "${pageContext.request.contextPath}/back/messageCenter/deleteMessageCenter.action",
									data: {ids:ids},
									success: function(data){
										if(data.isSuccess=="true"){
											$("#tt").datagrid("clearSelections");//删除后取消所有选项
...
function showDetailInfo(id){
	...
	$.ajax({
		...
		data: {id:id},
		success: function(data){
			$("#view_toMemberId").html(data.messageCenter.toMemberName);
			...
			if(data.messageCenter.messageSendState==1){
				$("#view_messageSendState").html("已发送");
			}
	   ...
	   <table   style="border:1px solid #99BBE8;text-align: center;" width="100%">
	    ...
	    <!-- 数据表单   -->
  		...
  		<div id="win" style="display:none;overflow-y:auto;scrollbar-face-color:#C4E1FF;">
		  	<jsp:include page="addMessage.jsp"></jsp:include>
