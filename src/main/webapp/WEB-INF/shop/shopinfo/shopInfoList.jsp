...
<html xmlns="http://www.w3.org/1999/xhtml">
					...
					handler:function(){
						...
						if(rows.length==0){//没有选择修改项
							...
							if (rows){//判断是否找到当前行，找到后异步形式从数据库中删除当前记录
								...
								createWindow(900,500,"&nbsp;&nbsp;审核入驻机构","icon-tip",false,"passWin");
				...
				handler:function(){
					...
					if(rows.length==0){//没有选择修改项
						...
						if (rows){//判断是否找到当前行，找到后异步形式从数据库中删除当前记录
							...
							createWindow(900,500,"&nbsp;&nbsp;审核机构及企业信息","icon-tip",false,"passWin");
							$("input.button_save").removeAttr("disabled");
							$("#addOrEditWin").css("display","none");
							$("#detailWin").css("display","none");
							$("#passWin").css("display","");
			...
			%>
				...
				%>
				{
					...
					handler:function(){
						var rows = $("#tt").datagrid("getSelections");//找到所有选中的行
						if(rows.length==0){//没有选择修改项
							msgShow("系统提示", "<p class='tipInfo'>请选择入驻机构机构！</p>", "warning");  
						}if(rows.length>1){//超过了一个选择项
							msgShow("系统提示", "<p class='tipInfo'>只能够选择一项！</p>", "warning");  
						}else if(rows.length==1){
  ...
  <div class="main">
		<table class="searchTab">
			<tr>
				<td class="search_toright_td" style="width:80px;'">机构账号：</td>
