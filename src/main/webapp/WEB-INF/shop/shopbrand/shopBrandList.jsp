  ...
  <head>
    ...
    $(function(){
		   $("#tt").datagrid({//数据表格
				...
				url:"${pageContext.request.contextPath}/back/shopBrand/listShopBrand.action",
				queryParams:{pageSize:pageSize},
				idField:"shopBrandId",//唯一性标示字段
				frozenColumns:[[//冻结字段
								...
								if(data){//判断是否删除
									if (rows){//判断是否找到当前行，找到后异步形式从数据库中删除当前记录
										...
										for(i=0;i<rows.length;i++){
											if(i==rows.length-1)ids+=rows[i].shopBrandId;
											else ids+=rows[i].shopBrandId+",";
										}
										$.ajax({
										   type: "POST",dataType: "JSON",
										   url: "${pageContext.request.contextPath}/back/shopBrand/delete.action",
										   data: {ids:ids},
...
//         }
  ...
  <style type="text/css">
	   	 ...
	   	 .querybtn a{height:22px; font-size:14px; margin-top: 5px;width:70px;border: 1px #4F9D9D solid; background:#ECF5FF;font-weight: bolder;color:#003D79;}
	</style>  
  <body>
<!--   		<span class="querybtn" style="font-size: 15px;"> -->
<!--   			品牌名称 ：<input type="text" style="height: 23px;width: 160px;" id="qbrandName" name="brandName" />&nbsp;&nbsp; -->
<!--   		</span> -->
<!-- 	    <span class="querybtn"> -->
