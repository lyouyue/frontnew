    ...
    $(function(){
		   ...
		   var type = '<s:property value="type"/>';
		    $("#form1").form({
		        ...
		        onSubmit:function(){
		        	var num=$("input[type=checkbox][name=brands][checked]").length;
					if(num==0){
		   ...
		   $("#tt").datagrid({//数据表格
				...
				columns:[[//未冻结字段
		            {field:"name",title:"品牌ID",width:120, formatter:function(value,rowData,rowIndex){  //function里面的三个参数代表当前字段值，当前行数据对象，行号（行号从0开始）
				...
				toolbar:[{//工具条
					...
					iconCls:"icon-add",
					handler:function(){
						createWindow(800,500,"&nbsp;&nbsp;添加品牌","icon-add",false,"win");
						$("#form1").css("display","");
    	...
    	function getBrandList(){
			 ...
			 $.ajax({//获取MES牌号信息
				   type: "POST",
				   url: "${pageContext.request.contextPath}/back/rightShow/listBrand.action",
				   data: {ptId:ptId},
				   dataType: "JSON",
				   success: function(data){
					   ...
					   for(i=0;i<data.length;i++){
						   htmlStr+="<tr><td width='8px' class='toleft_td'><input type='checkbox' onclick='checkedOne(this)' class='brands' id='"+i+"' name='brands' value='"+data[i].brandId+"_"+data[i].brandName+"'></td>"+
						   "<td width='250px' class='toleft_td'>&nbsp;&nbsp;"+data[i].brandName+"</td><td width='150px' class='toleft_td'>&nbsp;&nbsp;"+
